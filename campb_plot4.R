...
plot2<-ggplot(power.sub, aes(dt, Voltage)) + geom_line() + xlab("datetime") + ylab("Voltage") + theme(panel.grid.minor=element_blank(),panel.grid.major=element_blank(),panel.background=element_rect(fill="white",color="black"),axis.text=element_text(color="black")) + scale_x_datetime(breaks=date_breaks("1 day"),labels = date_format("%a"))
plot3<-ggplot(sub.meter,aes(x=dt,y=sub_metering,group=group, color=group)) + geom_line() + xlab("") + ylab("Energy sub metering") + theme(legend.title=element_blank(),panel.grid.minor=element_blank(),panel.grid.major=element_blank(),panel.background=element_rect(fill="white",color="black"),axis.text=element_text(color="black"),legend.position=c(1,1), legend.justification = c(1, 1)) + scale_x_datetime(breaks=date_breaks("1 day"),labels =date_format("%a")) + scale_color_manual(values=c("black","red","blue"))
plot4<-ggplot(power.sub, aes(dt, Global_reactive_power)) + geom_line() + xlab("datetime") + ylab("Global_rective_power (kilowatts)") + theme(panel.grid.minor=element_blank(), panel.grid.major=element_blank(),panel.background=element_rect(fill="white",color="black"),axis.text=element_text(color="black")) + scale_x_datetime(breaks=date_breaks("1 day"),labels = date_format("%a"))
